...
BEGIN
  ...
  FOR fk_rec IN
    ...
    WHERE con.contype = 'f'
      ...
      AND cls.relname = 'ficha_placa_intervencoes'
      AND att.attname = 'user_id'
  LOOP
    EXECUTE format('ALTER TABLE public.ficha_placa_intervencoes DROP CONSTRAINT %I', fk_rec.conname);
  END LOOP;
  ...
  -- Create the correct FK (auth.users) with ON DELETE SET NULL
  ALTER TABLE public.ficha_placa_intervencoes
    ADD CONSTRAINT fk_ficha_placa_intervencoes_user
    FOREIGN KEY (user_id)
