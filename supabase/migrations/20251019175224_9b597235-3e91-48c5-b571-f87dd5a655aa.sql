-- =====================================================
-- FUNCTION: Remover Duplicatas Mantendo o Primeiro
-- Criar APENAS a função primeiro, depois limpar, depois criar constraints
-- =====================================================
...
BEGIN
  ...
  v_partition_cols := CASE p_tabela
    ...
    WHEN 'necessidades_placas' THEN 'rodovia_id, COALESCE(km, 0), COALESCE(codigo, ''''), COALESCE(tipo_placa, ''''), COALESCE(lado, ''''), servico'
    WHEN 'necessidades_porticos' THEN 'rodovia_id, COALESCE(km, 0), COALESCE(tipo, ''''), COALESCE(vao_horizontal_m, 0), servico'
