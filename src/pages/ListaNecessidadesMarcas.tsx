...
export default function ListaNecessidadesMarcas() {
  ...
  const [selectedNecessidade, setSelectedNecessidade] = useState<NecessidadeMarca | null>(null);
  const [filterDecision, setFilterDecision] = useState<string>('TODOS');

  const { data: necessidades, isLoading } = useQuery({
    queryKey: ['necessidades-marcas-lista', loteId, rodoviaId],
    queryFn: async () => {
      let query = supabase
      ...
      return (data || []) as NecessidadeMarca[];
    },
  ...
  const necessidadesFiltradas = (necessidades || []).filter(nec => {
    const matchSearch = searchTerm === '' || 
      nec.tipo_demarcacao?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      nec.cor?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">➖ Lista de Necessidades - Marcas Longitudinais</h1>
            ...
            <CardTitle className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <div className="relative w-64">
                  ...
                  <Input
                    placeholder="Buscar tipo, cor ou KM..."
                    value={searchTerm}
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm">
                            <div>
                              ...
                              <p className="font-medium">{nec.tipo_demarcacao}</p>
                            </div>
                            <div>
                              <span className="text-muted-foreground">Cor:</span>
                              <p className="font-medium">{nec.cor}</p>
                            </div>
                            <div>
