...
export default function ListaNecessidadesTachas() {
  ...
  const [selectedNecessidade, setSelectedNecessidade] = useState<NecessidadeTacha | null>(null);
  const [filterDecision, setFilterDecision] = useState<string>('TODOS');

  const { data: necessidades, isLoading } = useQuery({
    queryKey: ['necessidades-tachas-lista', loteId, rodoviaId],
    queryFn: async () => {
      let query = supabase
      ...
      return (data || []) as NecessidadeTacha[];
    },
  ...
  const necessidadesFiltradas = (necessidades || []).filter(nec => {
    const matchSearch = searchTerm === '' || 
      nec.corpo?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      nec.cor_refletivo?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">💎 Lista de Necessidades - Tachas Refletivas</h1>
            ...
            <CardTitle className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <div className="relative w-64">
                  ...
                  <Input
                    placeholder="Buscar corpo, cor ou KM..."
                    value={searchTerm}
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm">
                            <div>
                              ...
                              <p className="font-medium">{nec.corpo}</p>
                            </div>
                            <div>
                              <span className="text-muted-foreground">Cor Refletivo:</span>
                              <p className="font-medium">{nec.cor_refletivo}</p>
                            </div>
                            <div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-6 pt-6 border-t space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                <strong>Limiar: 30%</strong> - Overlap mínimo para match direto ou substituição.
              </p>
              <p className="text-xs text-muted-foreground">
