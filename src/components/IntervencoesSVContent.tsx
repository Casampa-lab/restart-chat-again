  ...
  const fetchIntervencoes = async () => {
    try {
        ...
        .select(`
          *,
          placa:ficha_placa!ficha_placa_intervencoes_ficha_placa_id_fkey(
            codigo, tipo, lado, largura_m, altura_m, suporte, tipo_secao_suporte, substrato
          )
        `)
        .eq("user_id", user?.id)
      ...
      const mapped = (data || []).map((item: any) => ({
        ...
        dimensoes: item.placa 
          ? `${(item.placa.largura_m * 1000).toFixed(0)}x${(item.placa.altura_m * 1000).toFixed(0)}` 
          : item.dimensoes || null,
        material: item.material || item.placa?.substrato || null,
        tipo_suporte: item.tipo_suporte || item.placa?.suporte || null,
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label>Lado</Label>
                  <Select
                    onValueChange={(value) => setIntervencaoToEdit({...intervencaoToEdit, lado: value})}
                    defaultValue={intervencaoToEdit.lado}
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    onValueChange={(value) => setIntervencaoToEdit({...intervencaoToEdit, estado_conservacao: value})}
                    defaultValue={intervencaoToEdit.estado_conservacao}
                  >
                    <SelectTrigger className="w-full">
...
export default IntervencoesSVContent;
